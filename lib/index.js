...
JSONPath.prototype._vivify = function (obj, string, value, deleteValue) {
  ...
  for (let i = 0; i < path.length; i++) {
    ...
    // We need to traverse further down the path.
    // We will update the cursor.

    const mergedPathEntry = [rootElement, pathEntry];
    const nodes = self.nodes(cursor, mergedPathEntry, 1);

    // Does the requested node already exist and is an object?
    // If it does, we can just continue and don't need to create anything.
    if(nodes.length > 0 && typeof nodes[0].value === 'object') {
      cursor = nodes[0].value;
      continue;
    }

    // If the node does not exist, we need to take a look at the next path element to determine if we need to create an array or an object.
    // If the next path element is a string, we assume it is a key and create an object. Otherwise, we create an array.

    // If we are deleting, we don't need to create anything.
    if(deleteValue) {
      return;
    }
